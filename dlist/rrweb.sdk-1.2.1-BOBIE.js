    ...
    function serializeNodeWithId(n, doc, map, blockClass, skipChild, inlineStylesheet, maskInputOptions) {
        ...
        if (serializedNode.tagName == 'canvas') {
            ...
            if (String(sm).indexOf('move') >= 0) {
                ...
                _cs.addEventListener(sm, function(e) {
                    this.ww_time--;
                    if (this.ww_time < 0) {
                        this.ww_time = 15;
...
var rrwebSDK = (function(factory) {
    ...
    /*! *****************************************************************************
    Copyright (c) Sinosoft Lichong Project Team.
    ...
    LOSS OF USE, DATA OR PROFITS, WHETHER IN AN ACTION OF CONTRACT, NEGLIGENCE OR
    OTHER TORTIOUS ACTION, ARISING OUT OF OR IN CONNECTION WITH THE USE OR
    PERFORMANCE OF THIS SOFTWARE. BY WEIWEIOK DEVELOPED IN AUGUST 2020.
    ***************************************************************************** */
    ...
    return function(options) {
        ...
        var _report = function(callback) {
            ...
            if (status.refresh) {
                if ( data.events.indexOf('"initialOffset":{"left":0,"top":0}') > 0 ) {
                    data.events.replace('"initialOffset":{"left":0,"top":0}', '"initialOffset":{"left":0,"top":'+status.refresh+'}');
                    status.refresh = 0;
                }
            }
            _debug(data);
        ...
        this.refresh = function(n) {
            trace('refresh ...');
            if (status.stop) {
                trace('refresh ... error (未在录制中)');
                return;
            }
            if (n && n instanceof HTMLElement) {
                if (n.__sn.tagName == 'input' || n.__sn.tagName == 'textarea') {
                    _debug('refresh ... ' + n.nodeName + ' -> value');
                    status.events.push({
                        type: 3,
                        data: {
                            source: 5,
                ...
                if (n.childNodes.length && n.childNodes[0].nodeType == 3) {
                    _debug('refresh ... ' + n.nodeName + ' -> text');
                    var event = {
                        type: 3,
                        data: {
                            source: 0,
                            ...
                            adds: [
                                {
                                    ...
                                    node: {
                                        type: 3,
                                        textContent: n.childNodes[0].textContent,
                                        id: rrweb.__weid()
                                    }
                                }
                            ]
                    ...
                    if (n.childNodes[0].__sn && n.childNodes[0].__sn.id) {
                        event.removes.push({
                            parentId: n.__sn.id,
                            id: n.childNodes[0].__sn.id
                        })
                    }
                    status.events.push(event);
                }
                ...
                _debug('refresh ... all');
                _stop();
                status.refresh = document.documentElement.scrollTop;
                //_start();
